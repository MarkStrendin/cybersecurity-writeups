...
```
I wasn't espeically interested in fighting my antivirus over the decryption code, and probably shouldn't run arbitrary exploit code anyway, so I went to [CyberChef](https://gchq.github.io/CyberChef) to decrypt this. Using the RC4 operation with the passphrase set to `UTF8`, the Input format set to `Hex`, and output set to `UTF8`, the passphrase and ciphertext combination gets us:
